...
//    }
    ...
    fun encode(input: ByteArray): String {
        ...
        // Count leading zeros.
        var zeros = input.takeWhile { it == 0.toByte() }.count()

        // Copy of input since we modify it in-place
...
//    }
    ...
    fun decode(input: String): ByteArray {
        ...
        // Initialize INDEXES somewhere within Base58 object or class
        // Assuming INDEXES is an IntArray with appropriate mappings

        // Convert the base58-encoded ASCII chars to a base58 byte sequence (base58 digits).
        val input58 = ByteArray(input.length) { index ->
            val c = input[index]
            val digit = if (c.code < 128) INDEXES[c.code] else -1
            if (digit < 0) {
                throw AddressFormatException("Illegal character $c at position $index")
            }
            digit.toByte()
        }

        // Count leading zeros.
        var zeros = input58.takeWhile { it == 0.toByte() }.count()
...
//    }
    @Throws(AddressFormatException::class)
    fun decodeToBigInteger(input: String): BigInteger {
        val decodedByteArray = decode(input) // Assuming `decode` returns a ByteArray
        // Convert ByteArray to BigInteger. In Ionspin's BigInteger, use `fromByteArray` method
        return BigInteger.fromByteArray(decodedByteArray, Sign.POSITIVE)
    }
...
//        if (decoded.size < 4) throw AddressFormatException("Input too short")
...
//        if (decoded.size < 4) throw AddressFormatException("Input too short")
//
//        // Use Kotlin's sliceArray instead of Arrays.copyOfRange
//        val data = decoded.sliceArray(0 until decoded.size - 4)
//        val checksum = decoded.sliceArray(decoded.size - 4 until decoded.size)
//
//        // Assuming Sha256Hash.hashTwice returns a ByteArray
//        val actualChecksum = Sha256Hash.hashTwice(data).sliceArray(0 until 4)
//
//        // Use contentEquals for array comparison in Kotlin
//        if (!checksum.contentEquals(actualChecksum)) {
//            throw AddressFormatException("Checksum does not validate")
//        }
//
//        return data
