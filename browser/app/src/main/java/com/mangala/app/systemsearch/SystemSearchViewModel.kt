...
import com.mangala.app.onboarding.store.isNewUser
...
class SystemSearchViewModel(
    ...
    private var latestQuickAccessItems: Suggestions.QuickAccessItems =
        Suggestions.QuickAccessItems(emptyList())
    ...
    init {
        ...
        viewModelScope.launch {
            favoritesRepository.favorites().collect { favorite ->
                latestQuickAccessItems = Suggestions.QuickAccessItems(favorite.map {
                    FavoritesQuickAccessAdapter.QuickAccessFavorite(it)
                })
    ...
    fun userTappedDax() {
        viewModelScope.launch {
            ...
            command.value = Command.LaunchMangala
        }
    }
