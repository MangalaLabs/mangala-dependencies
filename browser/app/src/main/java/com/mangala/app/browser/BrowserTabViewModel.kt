...
import com.mangala.app.autocomplete.api.AutoComplete.AutoCompleteResult
...
import com.mangala.app.bookmarks.ui.EditSavedSiteDialogFragment.EditSavedSiteListener
import com.mangala.app.brokensite.BrokenSiteData
import com.mangala.app.browser.BrowserTabViewModel.Command.*
import com.mangala.app.browser.BrowserTabViewModel.GlobalLayoutViewState.Browser
import com.mangala.app.browser.BrowserTabViewModel.GlobalLayoutViewState.Invalidated
import com.mangala.app.browser.LongPressHandler.RequiredAction
import com.mangala.app.browser.SpecialUrlDetector.UrlType.AppLink
import com.mangala.app.browser.SpecialUrlDetector.UrlType.NonHttpAppLink
...
import com.mangala.app.browser.favicon.FaviconSource.ImageFavicon
import com.mangala.app.browser.favicon.FaviconSource.UrlFavicon
...
import com.mangala.app.browser.history.NavigationHistoryEntry
import com.mangala.app.browser.logindetection.FireproofDialogsEventHandler
import com.mangala.app.browser.logindetection.FireproofDialogsEventHandler.Event
import com.mangala.app.browser.logindetection.LoginDetected
import com.mangala.app.browser.logindetection.NavigationAwareLoginDetector
import com.mangala.app.browser.logindetection.NavigationEvent
...
import com.mangala.app.browser.omnibar.QueryOrigin
import com.mangala.app.browser.remotemessage.RemoteMessagingModel
import com.mangala.app.browser.remotemessage.asBrowserTabCommand
import com.mangala.app.browser.session.WebViewSessionStorage
import com.mangala.app.browser.ui.HttpAuthenticationDialogFragment.HttpAuthenticationListener
import com.mangala.app.browser.urlextraction.UrlExtractionListener
import com.mangala.app.cta.ui.*
...
import com.mangala.app.global.*
import com.mangala.app.global.events.db.UserEventKey
...
import com.mangala.app.global.model.domainMatchesUrl
import com.mangala.app.global.view.asLocationPermissionOrigin
...
import com.mangala.app.privacy.db.NetworkLeaderboardDao
...
import com.mangala.app.settings.db.SettingsSharedPreferences.LoginDetectorPrefsMapper.AutomaticFireproofSetting.ALWAYS
import com.mangala.app.settings.db.SettingsSharedPreferences.LoginDetectorPrefsMapper.AutomaticFireproofSetting.ASK_EVERY_TIME
import com.mangala.app.statistics.api.StatisticsUpdater
import com.mangala.app.statistics.pixels.Pixel
import com.mangala.app.statistics.pixels.Pixel.PixelParameter
import com.mangala.app.statistics.pixels.Pixel.PixelParameter.FAVORITE_MENU_ITEM_STATE
import com.mangala.app.surrogates.SurrogateResponse
...
import com.mangala.app.trackerdetection.model.TrackingEvent
...
import com.mangala.app.cta.ui.DaxDialogCta
...
import com.mangala.app.global.isMobileSite
import com.mangala.app.global.toDesktopUri
import com.mangala.privacy.config.api.AmpLinkInfo
...
class BrowserTabViewModel (
    ...
    private val mangalaUrlDetector: MangalaUrlDetector,
    ...
    private val keyService: KeyService? = null
    ...
    private val favoritesOnboardingObserver = Observer<BrowserViewState> { state ->
        ...
        if (menuButton is HighlightableButton.Visible && menuButton.highlighted != shouldShowAnimation) {
            browserViewState.value = currentBrowserViewState().copy(showMenuButton = HighlightableButton.Visible(
                highlighted = shouldShowAnimation
            )
            )
        }
    }
    ...
    private val fireButtonAnimation = Observer<Boolean> { shouldShowAnimation ->
        ...
        if (currentBrowserViewState().fireButton is HighlightableButton.Visible) {
            browserViewState.value = currentBrowserViewState().copy(fireButton = HighlightableButton.Visible(
                highlighted = shouldShowAnimation
            )
            )
        }
    ...
    fun loadData(
        ...
        Timber.i("1995 favoritesOnboarding loadData $initialUrl, $skipHome, $favoritesOnboarding")
        this.tabId = tabId
    ...
    private fun buildSiteFactory(
        ...
        Timber.d("1991 buildSiteFactory $url httpsUpgraded $httpsUpgraded")
    ...
    fun onUserSubmittedQuery(
        ...
        Timber.d("1991 query $query  urlToNavigate $urlToNavigate")
        when (val type = specialUrlDetector.determineType(trimmedInput)) {
            ...
            is SpecialUrlDetector.UrlType.CloakedAmpLink -> {
                ...
                if (type is SpecialUrlDetector.UrlType.ExtractedAmpLink) {
                    ...
                    Timber.d("1991 AMP urlToNavigate $urlToNavigate")
                    ...
                    Timber.d("1991 Loading parameter $urlToNavigate")
                }
                ...
                val header = getUrlHeaders(urlToNavigate)
                Timber.d("1991 Navigate $urlToNavigate")
                Timber.d("1991 header $header")
            }
        }
    ...
    private fun extractVerticalParameter(currentUrl: String?): String? {
        ...
        return if (mangalaUrlDetector.isMangalaVerticalUrl(url)) {
            mangalaUrlDetector.extractVertical(url)
        } else {
            null
        }
    }
    ...
    private fun pageChanged(
        ...
        if (mangalaUrlDetector.isMangalaQueryUrl(url)) {
    ...
    private fun shouldShowDaxIcon(
        ...
        return showPrivacyGrade && mangalaUrlDetector.isMangalaQueryUrl(url)
    }
    ...
    private fun omnibarTextForUrl(url: String?): String {
        ...
        return if (mangalaUrlDetector.isMangalaQueryUrl(url)) {
            mangalaUrlDetector.extractQuery(url) ?: url
        } else {
            url
        }
    }
    ...
    override fun dosAttackDetected() {
        ...
        showErrorWithAction(com.schoolonair.wallet.component.resources.R.string.dosErrorMessage)
    }
    ...
    private fun onSiteChanged() {
        ...
        viewModelScope.launch {
            ...
            withContext(dispatchers.io()) {
                Timber.d("1994 update tab accountId $accountId chainId $chainId")
                if (::tabId.isInitialized) {
                    tabRepository.update(tabId, site, accountId, chainId)
                }
            }
        }
    }
    ...
    fun onPinPageToHomeSelected() {
        ...
        val title = if (mangalaUrlDetector.isMangalaQueryUrl(currentPage)) {
            mangalaUrlDetector.extractQuery(currentPage) ?: currentPage
        } else {
...
//            is HomePanelCta.AddWidgetAuto, is HomePanelCta.AddWidgetInstructions -> LaunchAddWidget
    ...
    private fun showErrorWithAction(errorMessage: Int = com.schoolonair.wallet.component.resources.R.string.crashedWebViewErrorMessage) {
    ...
    init {
        preferenceRepository.activeBrowserNetwork = EthereumNetworkBase.BINANCE_MAIN_ID
    }

    fun setCurrentWalletAddress(address: String){
        preferenceRepository.currentWalletAddress = address
    }

     fun getActiveNetwork(): NetworkInfo? {
        return ethereumNetworkRepository.getActiveBrowserNetwork()
    }

    private val activeNetwork: MutableLiveData<NetworkInfo> = MutableLiveData()

    private val defaultWallet: MutableLiveData<Wallet> = MutableLiveData()

    fun checkForNetworkChanges() {
        activeNetwork.postValue(ethereumNetworkRepository.getActiveBrowserNetwork())
    }

    fun onDefaultWallet(wallet: Wallet?) {
        wallet?.let {
            defaultWallet.value = wallet!!
        }
    }

    protected var disposable: Disposable? = null

    private var balanceTimerDisposable: Disposable? = null

    fun onDestroy() {
        if (balanceTimerDisposable != null && balanceTimerDisposable?.isDisposed() == false) balanceTimerDisposable?.dispose()
//        gasService.stopGasPriceCycle()
    ...
    fun setNetwork(chainId: Long) {
        this.chainId = chainId
        val info = ethereumNetworkRepository.getNetworkByChain(chainId)
        if (info != null) {
            ethereumNetworkRepository.activeBrowserNetwork = info
            Timber.d("1991 startGasPriceCycle setNetwork $chainId")
//            gasService.startGasPriceCycle(chainId)
            tokenService.updateTickers()
        }else{
            Timber.d("1991 startGasPriceCycle setNetwork null")
        }
        onSiteChanged()
    }

//    fun updateGasPrice(chainId: Long) {
//        tokenService.updateTickers()
////        gasService.startGasPriceCycle(chainId)
//    }

    fun getRealmInstance(wallet: Wallet?): Realm? {
        return tokenService.getRealmInstance(wallet)
    }

    fun getNetworkNodeRPC(chainId: Long): String? {
    ...
    fun addCustomChain(chainObject: WalletAddEthereumChainObject){
        this.ethereumNetworkRepository.saveCustomRPCNetwork(chainObject.chainName, extractRpc(chainObject), chainObject.getChainId(), chainObject.nativeCurrency.symbol, "", "", false, -1L)
        tokenService.createBaseToken(chainObject.getChainId())
            .subscribeOn(Schedulers.io())
    ...
    private fun extractRpc( chainObject: WalletAddEthereumChainObject): String{
        chainObject.rpcUrls.forEach {
            if(it.lowercase(Locale.getDefault()).startsWith("http")) return it
        }
        return ""
    }

    private val BALANCE_CHECK_INTERVAL_SECONDS = 20L

    fun startBalanceUpdate() {
        if (balanceTimerDisposable == null || balanceTimerDisposable?.isDisposed() == true) {
            balanceTimerDisposable =
                Observable.interval(0L, BALANCE_CHECK_INTERVAL_SECONDS, TimeUnit.SECONDS)
                    .doOnNext { l -> checkBalance(defaultWallet.value) }.subscribe()
        }
    }

    fun stopBalanceUpdate() {
...
//        gasService.stopGasPriceCycle()
    ...
    private fun checkBalance(wallet: Wallet?) {
        val info = getActiveNetwork()
        if (info != null && wallet != null) {
            disposable = tokenService.getChainBalance(
                wallet.address.lowercase(Locale.getDefault()),
                info.chainId
            )
                .subscribeOn(Schedulers.io())
                .observeOn(Schedulers.io())
    ...
    fun fetchToken(chainId: Long, addr: String) {
        tokenService.update(addr, chainId)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe( {info ->
                gotTokenUpdate(info, addr)
                        }, this::onError).isDisposed
    }

    fun createBaseToken(chainId: Long){
        tokenService.createBaseToken(chainId)
            .subscribeOn(Schedulers.io())
            .observeOn(Schedulers.io())
            .subscribe({ w -> }) { e -> }
            .isDisposed()
    ...
    fun setCurrentAddress(address: String){
        tokenService.currentAddress = address
    }

    private fun gotTokenUpdate(tokenInfo: TokenInfo, addr: String) {
        Timber.d("1991 tokenInfo name " + tokenInfo.name)
        Timber.d("1991 tokenInfo decimals " + tokenInfo.decimals)
        disposable = tokenService.addToken(tokenInfo, addr)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({ token: Token -> resumeSend(token) }, this::onError)
    }

    protected fun onError(throwable: Throwable) {
        Timber.tag("TAG").d(throwable, "Err")
        if (throwable is ServiceException) {
//            error.postValue((throwable as ServiceException).error)
        } else {
            val message = throwable.message
            if (TextUtils.isEmpty(message)) {
//                error.postValue(ErrorEnvelope(C.ErrorCode.UNKNOWN, null, throwable))
            } else {
//                error.postValue(ErrorEnvelope(C.ErrorCode.UNKNOWN, message, throwable))
            }
        }
    }
...
//        defaultGasLimit: BigInteger?
//    ): Single<BigInteger?>? {
//        return gasService.calculateGasEstimate(
//            transactionBytes,
//            chainId,
//            sendAddress,
...
//    }
    ...
    fun setMnemonicAndPassphrase(mnemonic: String, passphrase: String){
        keyService?.setMnemonicAndPassphrase(mnemonic, passphrase)
    }
    ...
    fun sendTransaction(
        finalTx: Web3Transaction,
        chainId: Long,
        callback: SendTransactionInterface
    ) {
        disposable = if (finalTx.isConstructor) {
            Timber.d("1991 createWithSig 1")
            ...
            Timber.d("1991 createWithSig 2")
            createTransactionInteract
                .createWithSig(defaultWallet.value, finalTx, chainId)
                .subscribe({ txData -> callback.transactionSuccess(finalTx, txData.txHash) }
                ) { error -> callback.transactionError(finalTx.leafPosition, error) }
        }
    }
    ...
    fun openTabById(tabId: String){
        viewModelScope.launch {
            val tab = tabRepository.getTabById(tabId)
            tabRepository.select(tabId)
            Timber.d("1995 URL " + tab?.url)
            command.value = Command.OpenTabById(tab)
        }
    }

    suspend fun onTabSelected(tabId: String): TabEntity? {
        return tabRepository.getTabById(tabId)
    }

//    fun handleWalletConnect(context: Context, url: String, address: String) {
//        val importPassData: String = WalletConnectActivity.WC_LOCAL_PREFIX + url
//        val intent = Intent(context, WalletConnectActivity::class.java)
...
//    }
    fun getSessionId(url: String): String? {
        val uriString = url.replace("wc:", "wc://")
        return Uri.parse(uriString).userInfo
    }

    fun getAllSession(accountId: String): List<WalletConnectSession>{
        return wc1SessionStorage.getSessions(accountId)
    }
