...
import com.mangala.app.global.rating.AppEnjoymentPromptOptions
...
import com.mangala.app.privacy.ui.PrivacyDashboardActivity.Companion.RELOAD_RESULT_CODE
...
class BrowserViewModel (
    ...
    private val dataClearer: AutomaticDataClearer,
        ...
        it?.let { state ->
            when (state) {
                ApplicationClearDataState.INITIALIZING -> {
                    Timber.i("1991 App clear state initializing")
    ...
    suspend fun onTabsUpdated(tabs: List<TabEntity>?) {
        if (tabs.isNullOrEmpty()) {
            Timber.i("1991 Tabs list is null or empty; adding default tab")
