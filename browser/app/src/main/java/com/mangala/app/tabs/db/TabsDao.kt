...
package com.mangala.app.tabs.db
...
abstract class TabsDao {
    ...
    @Transaction
    open suspend fun purgeDeletableTabsAndUpdateSelection() {
        deleteTabsMarkedAsDeletable()
        if (selectedTab() != null) {
            return
        }
    ...
    @Query("update tabs set url=:url, title=:title, viewed=:viewed, accountId=:accountId, chainId=:chainId where tabId=:tabId")
