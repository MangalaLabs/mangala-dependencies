...
package com.mangala.app.browser.useragent
...
//    @Named("defaultUserAgent")
    ...
    fun userAgent(
        ...
        if (isDomainInUserAllowList || !toggle.isFeatureEnabled(PrivacyFeatureName.UserAgentFeatureName) || shouldUseDefaultUserAgent) {
            return if (isDesktop) {
                defaultUserAgent.replace(AgentRegex.platform, desktopPrefix)
            } else {
                defaultUserAgent
            }
        }
    ...
    private fun getWebKitVersionOnwards(forDesktop: Boolean): String? {
        val matches =
            AgentRegex.webkitUntilSafari.find(defaultUserAgent) ?: AgentRegex.webkitUntilEnd.find(defaultUserAgent) ?: return null
...
val defaultUserAgentModule = module {
    single(named("defaultUserAgent")) {
        runCatching {
