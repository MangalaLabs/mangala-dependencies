...
import com.mangala.app.waitlist.email.WaitlistNotificationDialog
import com.mangala.app.global.view.html
import com.mangala.mobile.android.ui.spans.MangalaClickableSpan
...
import com.schoolonair.wallet.browser.app.databinding.FragmentEmailProtectionSignInBinding
...
class EmailProtectionSignInFragment : EmailProtectionFragment() {
    ...
    private val viewModel: EmailProtectionSignInViewModel by viewModel()
    private val binding: FragmentEmailProtectionSignInBinding by viewBinding()

    private val getNotificationSpan = object : MangalaClickableSpan() {
        override fun onClick(widget: View) {
            showNotificationDialog()
        }
    }

    private val readBlogSpan = object : MangalaClickableSpan() {
        override fun onClick(widget: View) {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_email_protection_sign_in, container, false)
    }
    ...
    private fun renderErrorMessage() {
        ...
        Toast.makeText(context, com.schoolonair.wallet.component.resources.R.string.emailProtectionErrorJoiningWaitlist, Toast.LENGTH_LONG).show()
    }
    ...
    private fun renderInBeta() {
        binding.headerImage.setImageResource(com.schoolonair.wallet.component.resources.R.drawable.ic_gift_large)
        ...
        binding.statusTitle.text = getString(com.schoolonair.wallet.component.resources.R.string.emailProtectionStatusTitleInBeta)
        setClickableSpan(binding.emailPrivacyDescription, com.schoolonair.wallet.component.resources.R.string.emailProtectionDescriptionInBeta, listOf(readBlogSpan))
    }

    private fun renderJoinedQueue(notify: Boolean) {
        binding.headerImage.setImageResource(com.schoolonair.wallet.component.resources.R.drawable.ic_list)
        binding.waitListButton.gone()
        binding.inviteCodeButton.show()
        binding.getStartedButton.gone()
        binding.duckAddressButton.show()
        binding.footerDescription.gone()
        binding.statusTitle.text = getString(com.schoolonair.wallet.component.resources.R.string.emailProtectionStatusTitleJoined)
        if (notify) {
            setClickableSpan(binding.emailPrivacyDescription, com.schoolonair.wallet.component.resources.R.string.emailProtectionDescriptionJoinedWithNotification, listOf(readBlogSpan))
        } else {
            setClickableSpan(
                binding.emailPrivacyDescription,
                com.schoolonair.wallet.component.resources.R.string.emailProtectionDescriptionJoinedWithoutNotification,
    ...
    private fun renderNotJoinedQueue() {
        binding.headerImage.setImageResource(com.schoolonair.wallet.component.resources.R.drawable.contact_us)
        ...
        binding.duckAddressButton.show()
        binding.waitListButton.isEnabled = true
        binding.statusTitle.text = getString(com.schoolonair.wallet.component.resources.R.string.emailProtectionStatusTitleJoin)
        setClickableSpan(binding.emailPrivacyDescription, com.schoolonair.wallet.component.resources.R.string.emailProtectionDescriptionJoin, listOf(readBlogSpan))
    }

    private fun setClickableSpan(
        view: TextView,
        stringId: Int,
        span: List<MangalaClickableSpan>
    ) {
        context?.let {
    ...
    private fun openBrowserTab(url: String) {
        context?.let {
            startActivity(BrowserActivityNavigationUtils.intent(it, url))
            activity?.finish()
        }
    }
